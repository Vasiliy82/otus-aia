...
- ADR: LLM Serving — выбор между vLLM / SGLang / TGI после закупки GPU
- ADR: Graph DB — переход с PostgreSQL на Neo4j cluster
- ADR: Observability — OpenTelemetry + Jaeger + Prometheus
- ADR: IdP Integration — подключение Active Directory для RBAC
